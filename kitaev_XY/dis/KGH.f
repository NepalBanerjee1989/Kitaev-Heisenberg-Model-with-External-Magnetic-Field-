...
C***************************************************************************C
          ...
          REAL*8 pi,th,phi,S
          PARAMETER(L=40,LSQ=L*L,itmx=200000,itequi=100000,
     &    itav=itmx-itequi,pi=4*atan(1.d0),LMAX=L) 
          ...
          REAL*8 t,EI,EF,dE,prob,am1,am2,am3,energy1,energy2
          REAL*8 xm,xm1,xm2,xm3,en,flucM,flucE,UL
          REAL*8 hx,hy,hz
          REAL*8 r1,r2,r3,r,g,p,SMAG
...
C===================================================C
          ...
          OPEN(1,file='KGH.ini',status='old')
            ...
            read(1,*) hx,hy,hz          !Hx,Hy,Hz 
            read(1,*) g                 !Tunning the U1 --> Z2 transition
            read(1,*) p                 !Filling factor
          CLOSE(1)
...
C*********Screen Output *******************************
     ...
     &     X3J3,KX,KY,KZ,GamaX,GamaY,GamaZ,hx,hy,hz,g,p,LSQ,itmx,itequi
           WRITE(*,22)
...
C----------------------------------C
  ...
  31     Format(/,5X,50('-'),/,
     ...
     &20X,'Hx,Hy,Hz:',F5.4,',',F5.4,',',F5.4/,      
     &20X,'Tunning parameter(g):',F5.2/,    
     &20X,'Filling Factor(p):',F5.2/,    
...
C************************************************************C
C*** Simulating Initial Spin Config with a filling factor ***C
C************************************************************C
         DO i=1,L
           DO j=1,L
...
C******** Main Simulation Loop Start Here *******
        ...
        DO temp=tmax,tmin,-dt       ! Temperature Loop 
           ...
           energy2=0.d0 ;
            ...
            SMAG=SQRT(sx(i,j)**2 + sy(i,j)**2 + sz(i,j)**2)

            IF(SMAG.NE.0.d0)THEN
...
C**** Calculate the energy of the Chosen spin *************C
        ...
        EI=-sx(i,j)*X1J1*(sx(ip,j)+sx(in,j)+sx(i,jl)) !J1(NN)
...
C*********External Magnetic Field *************************
     ...
     & -(hx*sx(i,j)+hy*sy(i,j)+hz*sz(i,j))         !Hx,Hy,Hz   
...
C**** Calculate the spin energy after flipping *********C
      EF=-flipx(i,j)*X1J1*(sx(ip,j)+sx(in,j)+sx(i,jl))      !J1(NN)
...
C******** External Magnetic Field *************************
     ...
     &-(hx*flipx(i,j)+hy*flipy(i,j)+hz*flipz(i,j))          !Hx,Hy,Hz    
